...
class Logger {
  ...
  constructor() {
    this.envVars = new LoggerEnvVars();
    this.level = this.envVars.get('LOG_LEVEL') || (process.env.NODE_ENV === 'production' ? 'info' : 'trace');
    this.logger = null;
  ...
  private getTransports() {
    ...
    const fileFormat = winston.format.combine(errorFormatter(), winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), winston.format.json());
    const consoleFormat = winston.format.combine(
      errorFormatter(),
      winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }),
      winston.format.printf(({ level, message, timestamp, stack }) => {
        // If there's a stack (i.e., it's an Error), show that too
        return stack ? `[${level}]: ${timestamp} ${message}\n${stack}` : `[${level}]: ${timestamp} ${message}`;
      }),
